...
python step3b_sa_refine.py \
  --sens_csv ./artifacts_8/bitmin/step1/layerwise_sensitivity.csv \
  --alpha_csv ./artifacts_8/bitmin/step2/alpha_layerwise_rank64.csv \
  --mode budget --avg_bits 2.50 \
  --steps_per_temp 2000  --max_temps 400   --rho 0.995   --restarts 2 \
  --output_dir ./artifacts_8/bitmin/step3b_budget
...
def load_sens_table(path: str, sens_col: str = "C_mean_per_batch") -> List[Layer]:
    ...
    if not found_name:
        raise ValueError("Cannot find a name column (layer_name/module/name) in sens_csv")

    # detect weight
    found_w = next((c for c in weight_cols if c in rows[0]), None)
    if not found_w:
        raise ValueError("Cannot find a weight column among: %s" % ", ".join(weight_cols))
...
def load_alpha_table(path: str) -> Dict[str, Dict[int, float]]:
    ...
    for r in rows:
        ...
        mod_key = keys.get('module', None) or keys.get('layer_name', None) or keys.get('name', None)
...
def load_loss_table(path: str) -> Dict[str, Dict[int, float]]:
    ...
    found_triplet = next((tpl for tpl in candidates if all(t in rows[0] for t in tpl)), None)
...
def waterfill_budget(Cp: List[float], w: List[int], B: float, Rmin: float, Rmax: float) -> List[float]:
...
def waterfill_target_find_budget(Cp: List[float], w: List[int], L_tgt: float, Rmin: float, Rmax: float) -> Tuple[float, List[float]]:
...
def propose_swap_budget(problem: Problem, b: List[int], rng: random.Random) -> Tuple[Optional[Tuple[int,int]], float]:
...
def auto_T0_budget(problem: Problem, b: List[int], rng: random.Random, samples: int = 256) -> float:
    deltas = []
...
def energy_target(problem: Problem, b: List[int], lam: float, L_tgt: float) -> Tuple[float, float, int]:
    L = total_loss(problem, b)
...
def propose_move_target(problem: Problem, b: List[int], rng: random.Random, p_decrease: float = 0.7) -> Tuple[int, int]:
    min_b, max_b = problem.min_bit, problem.max_bit
    n = len(b)
    for _ in range(64):
        j = rng.randrange(n)
...
def auto_T0_target(problem: Problem, b: List[int], lam: float, L_tgt: float, rng: random.Random, samples: int = 256) -> float:
    deltas = []
...
def sa_target(
    ...
    best_b = b[:]; best_E, best_L, best_B = E, L, B
    ...
    for _ in range(max_temps):
        ...
        for _ in range(steps_per_temp):
            ...
            new_L = L + Ldiff; new_B = B + Bdiff
            new_E = float(new_B) + lam * max(0.0, new_L - L_tgt)
            Ediff = new_E - E
            if Ediff < 0 or rng.random() < math.exp(-Ediff / max(1e-12, T)):
                b[j] = new_b; E, L, B = new_E, new_L, new_B
                accept_cnt += 1
                if E < best_E - 1e-12 or (abs(E - best_E) < 1e-12 and L <= L_tgt and new_B < best_B):
...
def main():
    ap = argparse.ArgumentParser(description="Step3b — SA refinement for integer bit allocation (folder outputs)")
    ap.add_argument('--sens_csv', type=str, required=True,
                    help='CSV with layer_name, w_j (or numel), C_j (or Cprime_j)')
    ap.add_argument('--sens_col', type=str, default='C_mean_per_batch',
                    help='Sensitivity column to use (default: C_mean_per_batch). Alternatives auto-detected.')

    ap.add_argument('--loss_table_csv', type=str, default=None,
                    help='Measured per-layer loss table CSV with columns b2,b3,b4 (or L2,L3,L4). Preferred if provided.')
    ap.add_argument('--alpha_csv', type=str, default=None, help='Optional alpha table: module,bit,alpha')
    ap.add_argument('--alpha_default', type=float, default=1.0, help='Fallback alpha when missing (default: 1.0)')

    ap.add_argument('--init_assign_csv', type=str, default=None,
                    help='Optional seed assignment CSV with layer_name and b_init or R_int')

    ap.add_argument('--mode', type=str, default='budget', choices=['budget','target'], help='Optimization mode')
    ap.add_argument('--avg_bits', type=float, default=None, help='(budget) average bits per weight')
    ap.add_argument('--total_bits', type=float, default=None, help='(budget) total bit budget Σ w_j b_j')
    ap.add_argument('--target_loss', type=float, default=None, help='(target) target total loss L_tgt')

    ap.add_argument('--bits', type=str, default='2,3,4', help='Allowed bits, comma-separated (default: 2,3,4)')

    ap.add_argument('--T0', type=str, default='auto', help='Initial temperature (float) or "auto"')
    ap.add_argument('--rho', type=float, default=0.995, help='Cooling factor (default: 0.995)')
    ap.add_argument('--steps_per_temp', type=int, default=50, help='SA steps per temperature (default: 50)')
    ap.add_argument('--max_temps', type=int, default=400, help='Max temperature epochs (default: 400)')
    ap.add_argument('--restarts', type=int, default=2, help='Number of SA restarts (default: 2)')
    ap.add_argument('--seed', type=int, default=0, help='RNG seed (default: 0)')

    # 출력 방식: Step3와 동일하게 폴더 중심
    ap.add_argument('--output_dir', type=str, required=True, help='Directory to write outputs (report, bit_assign, meta)')
    ...
    loss_table, alpha_global = build_loss_table(
        layers=layers, bits=bits, loss_table_csv=args.loss_table_csv,
        alpha_csv=args.alpha_csv, alpha_default=args.alpha_default,
    )

    problem = Problem(layers=layers, bits_set=bits, min_bit=min_bit, max_bit=max_bit, loss_table=loss_table)
    ...
    if args.mode == 'budget':
        ...
        R_cont = waterfill_budget(Cprime, w, B, Rmin=float(min_bit), Rmax=float(max_bit))
        ...
        if args.init_assign_csv and os.path.exists(args.init_assign_csv):
            ...
            if abs(curB - B) > 1e-6:
                b_seed = round_and_fix_budget([float(x) for x in b_seed], w, loss_table, bits, B)

        T0 = auto_T0_budget(problem, b_seed, rng) if str(args.T0).lower() == 'auto' else float(args.T0)

        best_b, best_L = None, float('inf')
        for _ in range(max(1, args.restarts)):
            bb, LL = sa_budget(problem, b_seed[:], T0=T0, rho=args.rho,
                               steps_per_temp=args.steps_per_temp, max_temps=args.max_temps, rng=rng)
            if LL < best_L:
                best_L, best_b = LL, bb
        final_b = best_b
        final_L = total_loss(problem, final_b)
        final_B = total_bits(problem, final_b)
        target_desc = f"avg_bits={args.avg_bits}" if args.avg_bits is not None else f"total_bits={args.total_bits}"
        ...
        B_star, R_star = waterfill_target_find_budget(Cprime, w, L_tgt, Rmin=float(min_bit), Rmax=float(max_bit))
        b_seed = round_and_fix_budget(R_star, w, loss_table, bits, B_star)

        lam0 = 1.0
        T0 = auto_T0_target(problem, b_seed, lam=lam0, L_tgt=L_tgt, rng=rng) if str(args.T0).lower() == 'auto' else float(args.T0)

        best_b, best_L, best_B = None, float('inf'), 10**30
        for _ in range(max(1, args.restarts)):
            bb, LL, BB = sa_target(problem, b_seed[:], L_tgt=L_tgt, lam0=lam0, T0=T0, rho=args.rho,
                                   steps_per_temp=args.steps_per_temp, max_temps=args.max_temps, rng=rng)
            if LL <= L_tgt:
    ...
    with open(report_path, 'w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow(['layer_name', 'w_j', 'L2', 'L3', 'L4', 'b_init_or_seed', 'b_final', 'loss_final', 'Δloss_final'])
        report_seed = locals().get('b_seed', final_b)
        for j, Lyr in enumerate(problem.layers):
            L2 = problem.loss_table[j][2]; L3 = problem.loss_table[j][3]; L4 = problem.loss_table[j][4]
            b0 = report_seed[j] if j < len(report_seed) else final_b[j]
            bf = final_b[j]; lf = problem.loss_table[j][bf]; dl = lf - problem.loss_table[j][b0]
            writer.writerow([Lyr.name, Lyr.w, f"{L2:.6e}", f"{L3:.6e}", f"{L4:.6e}", b0, bf, f"{lf:.6e}", f"{dl:.6e}"])
        Wsum = sum(w); avg_bits = float(final_B) / max(1.0, float(Wsum))
        writer.writerow(["__TOTAL__", Wsum, "", "", "", "", "", f"{final_L:.6e}", f"B={final_B}; avg_bits={avg_bits:.6f}"])
    ...
    if args.out_csv:
        ...
        with open(report_path, 'r', encoding='utf-8') as src, open(args.out_csv, 'w', encoding='utf-8') as dst:
            dst.write(src.read())
