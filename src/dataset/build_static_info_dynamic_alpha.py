...
Input:
  ...
  - alpha_csv: alpha table CSV (either wide: alpha2/alpha3/alpha4, or long: bit + alpha)
...
python dataset/build_static_info_dynamic_alpha.py \
  ...
  --output_dir ../artifacts/surrogate_data
...
def build_static_info(
    ...
    # keep only bits 2/3/4
    tmp = tmp[tmp[bit_col].isin([2, 3, 4])].copy()

    if tmp.empty:
        raise RuntimeError("[alpha] filtered alpha table is empty after keeping bits in {2,3,4}")
    ...
    for ln in sens_layers:
        ...
        alpha_map[str(ln)] = {"2": a2, "3": a3, "4": a4}

    # quick sanity: how many layers have (almost) identical alphas?
    eq_cnt = 0
    for ln in sens_layers:
        a2, a3, a4 = alpha_map[ln]["2"], alpha_map[ln]["3"], alpha_map[ln]["4"]
        if max(abs(a2 - a3), abs(a2 - a4), abs(a3 - a4)) < 1e-8:
            eq_cnt += 1
    ...
    static_info = {
        ...
        "meta": {
            ...
            "note": f"layers_with_identical_alpha_2_3_4 (tol=1e-8): {eq_cnt}/{len(sens_layers)}",
        },
    }
