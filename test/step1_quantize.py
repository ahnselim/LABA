...
저장(out_dir):
  ...
  - (opt) original_weights.pt  : dict["<full>.weight"] = W  [O,I] (cpu)
  - (opt) quantized_weights.pt : dict["<full>.weight"] = Wq [O,I] (fp16/cpu)
  - (opt) quant_error.pt       : dict["<full>.weight"] = (W - Wq) (fp32/cpu)

예시:
CUDA_VISIBLE_DEVICES=1 python step1_quantize.py \
  --model_id meta-llama/Llama-3.2-3B \
  --bits 4 --group_size 128 \
  --clip_percentile 0.0 \
  --lloyd_iter 12 \
  --out_dir ./output/4bit_asym_nonuniform_base \
  --save_wq --save_err
...
def main():
    ...
    ap.add_argument("--save_w", "--save_original", dest="save_w", action="store_true",
                    help="also save original W as original_weights.pt")
    ...
    for full_name, W_cpu in tqdm(state.items()):
        ...
        # test_uv_tail_energy.py uses original_weights.pt + quantized_weights.pt.
        # Save original weights automatically when Wq is requested for convenience.
        if args.save_w or args.save_wq:
            orig_weights[full_name] = W_cpu
        if args.save_wq:
    ...
    if args.save_w or args.save_wq:
        w_path = os.path.join(args.out_dir, "original_weights.pt")
        torch.save(orig_weights, w_path)
    if args.save_wq:
    ...
    if args.save_w or args.save_wq:
        print(f"  • {w_path}  ({len(orig_weights)} layers)")
    if args.save_wq:
