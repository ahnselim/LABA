...
# -*- coding: utf-8 -*-
"""
Step 4 (Triton Parameter Packing Version)
...
- 3/4bit: Asymmetric (step1/3 Triton 로직과 호환)
...
출력:
 • out_dir/quantized_model_triton.pt  # [신규] Triton 커널용 (W_packed, scales, zeros, quant_type)
...
CUDA_VISIBLE_DEVICES=1 python Triton/step4_pack_triton_params.py \
    --model_id meta-llama/Llama-3.2-3B \
    --bit_assign_csv ./artifacts_3/bitmin/step3b_budget/bit_assign.csv \
    --out_dir ./artifacts_3/bitmin/step4_budget_triton \
    --rank 64 \
    --group_size 128 \
...
@torch.no_grad()
def pack_weights(q_values_unpacked: torch.Tensor, bit: int, group_size: int) -> torch.Tensor:
    """
...
def pack_zeros(zeros_unpacked: torch.Tensor, bit: int) -> torch.Tensor:
    ...
    if bit == 4:
        ...
        # 패딩이 G % 2 != 0 일 경우를 대비
        if G % 2 != 0:
            zeros_unpacked = torch.nn.functional.pad(zeros_unpacked, (0, 1), "constant", 0)
            G += 1
        ...
        pad = (4 - G % 4) % 4
        if pad != 0:
            zeros_unpacked = torch.nn.functional.pad(zeros_unpacked, (0, pad), "constant", 0)
            G += pad
...
def get_params_uniform_asym(
    ...
    zeros_float = torch.where(scales > eps, (-minv / scales), 0.0).round().clamp(0.0, qmax)
    ...
    if orig_I % group_size != 0:
        bits_per_byte = 8 // b
        if b == 3: bits_per_byte = 2 # 3bit는 4bit 컨테이너(2개) 사용
...
def get_params_2bit_lloyd_sym(
    ...
    scales = torch.stack([alpha, beta], dim=-1).squeeze(-2).to(torch.float16) # Shape [O, G, 2]
...
def get_params_2bit_qtr_zero(
    ...
    mag_indices = (Y >= 0.5 * alpha).to(torch.uint8) # 1 if magnitude is alpha, 0 if zero
    sgn_indices = (X < 0).to(torch.uint8)            # 1 if negative, 0 if positive
...
def main():
    ...
    ap.add_argument("--reuse_calib", action="store_true", help="기존 Σ_x^{1/2} 캐시(.pt) 재사용")
    ...
    calib_basename = (
        f"calib_oas_sqrtdiag_{safe_model}__{safe_dataset}__{safe_config}__{args.split}__ns{args.nsamples}_L{args.seqlen}.pt"
    )
    ...
    if args.reuse_calib and os.path.exists(calib_path):
        ...
        cov_ops = estimate_diag_cov_oas_per_module(
            ...
            nsamples=args.nsamples, seqlen=args.seqlen,
            calib_dataset=args.dataset, calib_config=args.dataset_config,
            split=args.split, use_streaming=bool(args.use_streaming),
            matmul_dtype=torch.float32,
        )
