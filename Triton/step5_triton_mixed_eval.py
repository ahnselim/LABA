...
# -*- coding: utf-8 -*-
"""
Step 5 (True Quant Triton Mixed-Precision Version + Memory Check)
- ✨ [신규] 'main' 함수 시작 시 원본 FP16 모델을 로드하여 GPU 메모리 사용량 측정
- ✨ [신규] 퀀타이즈된 모델(Triton + SVD) 로드 후 GPU 메모리 사용량 측정
...
python step5_triton_mixed_eval.py \
    --model_name meta-llama/Llama-2-7b-hf \
    --quant_path ./artifacts/bitmin/step4_budget/quantized_model_triton.pt \
    --correction_path ./artifacts/bitmin/step4_budget/correction_layerwise.pt \
    --bmap_path ./artifacts/bitmin/step4_budget/b_ref_map_layerwise.json \
    --alpha_svd 1.0 \
    --device cuda:0 \
    --trust_remote_code
"""
...
def log_memory_stats(tag: str, device: torch.device):
    ...
    print(
        f"\n--- Memory Stats ({tag}) ---\n"
        f"  Device   : {dev_idx}\n"
        f"  Allocated: {_bytes_to_gib(allocated):.3f} GiB  <-- (이것이 실제 텐서 크기 합입니다)\n"
...
def triton_gemm_w4_asym_kernel(
    ...
    M, N, K,
    stride_xm, stride_xk,
    stride_qwm, stride_qwk,
    stride_qzm, stride_qzk,
    stride_sm, stride_sk,
    stride_om, stride_on,
    ...
    for k in range(0, tl.cdiv(K, BLOCK_SIZE_K)):
        ...
        qzeros_ptrs = qzeros_ptr + (offs_bn[None, :] * stride_qzm + zeros_group_id * stride_qzk)
        ...
        dequant_weights = (nibbles.to(tl.float32) - zeros.to(tl.float32)) * scales.to(tl.float32)
...
def triton_gemm_w2_qtr_sym_kernel(
    x_ptr, qweight_ptr, scales_ptr, bias_ptr, output_ptr,
    M, N, K,
    stride_xm, stride_xk,
    stride_qwm, stride_qwk,
    stride_sm, stride_sk,
    ...
    offs_k = tl.arange(0, BLOCK_SIZE_K)
    x_ptrs = x_ptr + (offs_am[:, None] * stride_xm + offs_k[None, :] * stride_xk)
    qweight_ptrs = qweight_ptr + (offs_bn[None, :] * stride_qwm + (offs_k[:, None] // 4) * stride_qwk)
    ...
    for k in range(0, tl.cdiv(K, BLOCK_SIZE_K)):
        ...
        qvals = tl.where(k_mod_4 == 0, (packed_weights & 0x03),
                tl.where(k_mod_4 == 1, (packed_weights >> 2) & 0x03,
                    tl.where(k_mod_4 == 2, (packed_weights >> 4) & 0x03,
...
def triton_gemm_w2_lloyd_sym_kernel(
    ...
    for k in range(0, tl.cdiv(K, BLOCK_SIZE_K)):
        ...
        group_id = k_offs[:, None] // group_size
        scales_base_ptrs = scales_ptr + (offs_bn[None, :] * stride_sm + group_id * stride_sk)
...
def gemm_w4_asym(x, qweight, qzeros, scales, bias, group_size):
    ...
    M, K = (original_shape[0] * original_shape[1], original_shape[2]) if x.dim() == 3 else x.shape
    N = scales.shape[0]
    x = x.reshape(M, K)
    ...
    triton_gemm_w4_asym_kernel[grid](
        x, qweight, qzeros, scales, bias, output,
        M, N, K,
...
def gemm_w2_lloyd_sym(x, qweight, scales, bias, group_size):
    ...
    triton_gemm_w2_lloyd_sym_kernel[grid](
        ...
        x.stride(0), x.stride(1),
        qweight.stride(0), qweight.stride(1),
        scales.stride(0), scales.stride(1), scales.stride(2), # Pass all 3 strides
        output.stride(0), output.stride(1),
        group_size=group_size,
        BLOCK_SIZE_M=64, BLOCK_SIZE_N=64, BLOCK_SIZE_K=32,
        HAS_BIAS=(bias is not None),
...
class AddABCorrection(nn.Module):
    def __init__(
        ...
        self.register_buffer("A_q", A_q.to(device=dev, dtype=dt, copy=True), persistent=False)
        self.register_buffer("B_q", B_q.to(device=dev, dtype=dt, copy=True), persistent=False)
...
def patch_ab_from_corrections(
    ...
    if bmap_keys_weight:
        filter_modules = set([k[:-7] for k in bmap_keys_weight if k.endswith(".weight")])
        target_modules = target_modules & filter_modules
        print(f"[AB] filtering by bmap: {len(filter_modules)} keys → {len(target_modules)} targets")
    ...
    for base in sorted(target_modules):
        ...
        if not isinstance(inner, TritonMixedBitLinear):
            missing_module += 1
            continue
    ...
    print(f"[AB] patched={patched}, missing_module={missing_module} (Triton layers skipped)")
    if examples:
...
# -----------------------------
def main():
    ap = argparse.ArgumentParser("Step 5 — True Quant Triton Mixed-Precision PPL Evaluation")
    ...
    ap.add_argument("--correction_path", required=True, help="Path to 'correction_layerwise.pt' (A,B matrices)")
    ap.add_argument(
    ...
    if not HAS_TRITON:
        print("Triton is required to run this script. Aborting.")
        return
    ...
    print("🔄 Injecting TritonMixedBitLinear modules...")
    ...
    for name, module in model.named_modules():
        ...
        if weight_name in target_layers:
            ...
            has_bias = bias_key in original_state_dict and original_state_dict[bias_key] is not None
            
            new_module = TritonMixedBitLinear(
                in_features=I,
                out_features=O,
                entry=entry,
                bias=has_bias
    ...
    print(f"✅ Injected {injected_count} TritonMixedBitLinear modules.")
    ...
    print("\n--- Evaluating Quant-Only (Triton W2/W3/W4) ---")
    ...
    # 5-2) Quant+AB (SVD 켬)
    print(f"\n--- Evaluating Quant + SVD (Triton W2/W3/W4 + A,B) ---")
    for m in model.modules():
        if isinstance(m, AddABCorrection):
    ...
    print("-" * 20)
    print(f"Quant-only PPL : {ppl_q:.4f} (Triton WxAy, α=0)")
    print(f"Quant+AB  PPL  : {ppl_ab:.4f} (Triton WxAy, α={args.alpha_svd})")
