...
(추가) 가중평균(파라미터수×비트) 예산 정확히 맞춤(정수 제약 고려해 gcd 정렬)
...
(추가) 모든 후보를 weighted budget(Σ w_j b_j = target)으로 프로젝션

동작 요약:
1) (Import) Step 1/2/3 유틸 + Step 5 PPL 평가 로직 이식
2) (Prebake) bit{2,3,4}/{layer}.pt에서 Wq/A/B 로드 후 W_eff=Wq+A@B로 패치해 PPL 측정
3) 빔 서치: Expand→(Proxy L(b) Filter)→(PPL Evaluate with LRU cache)→Select
4) 매 세대 best PPL 로깅. 수렴 시 종료.

# 사용할 GPU 지정
export CUDA_VISIBLE_DEVICES=1

CUDA_VISIBLE_DEVICES=1 \
python step3b_ppl_mc_converge.py \
  --model_id meta-llama/Llama-3.2-3B \
  --gpu_id 0 \
  --sens_csv ./artifacts_13/bitmin/step1/layerwise_sensitivity.csv \
  --alpha_csv ./artifacts_13/bitmin/step2/alpha_layerwise_rank64.csv \
  --prebake_root ./artifacts_13/bitmin/prebake_llama2_13b_r64_g128 \
  --init_assign_csv ./artifacts_13/bitmin/step3b_budget/bit_assign.csv \
  --avg_bits 2.50 \
  --beam_size 10 \
  --expansion_k 20 \
  --filter_p 80 \
  --generations 0 \                 # 무제한 (수렴조건으로 멈춤)
  --max_generations 0 \             # (선택) 상한 두려면 >0
  --converge_eps 1e-3 \
  --converge_rel_eps 1e-3 \
  --patience 12 \
  --stable_bits_patience 12 \
  --time_limit_sec 43200 \          # (선택) 12시간 안전장치
  --ppl_cache_max 20000 \
  --output_dir ./artifacts_13/bitmin/step3b_ppl_final_search_prebaked_convergence
...
def main(args):
    ...
    print(
        f"초기 빔 PPL: {beam[0][0]:.4f} | 초기 가중평균={weighted_sum_bits(beam[0][2], W_map)/sum_w:.6f}"
    )
    ...
    while True:
        ...
        for l_score, b_tuple in tqdm(finalists, desc=f"G-{gen} PPL 평가", leave=False):
            b_dict = dict(b_tuple)
            # 안전: 가중예산 프로젝션을 재적용(중간 변형 방지)
            b_dict = project_to_weighted_budget(
                b_dict, W_map, C_prime_filtered, B_target, args.bmin, args.bmax
            )
            ppl = ppl_eval.evaluate(b_dict)
    ...
    print(
        f"최종 Σ w·b = {S_final} / Σw={sum_w} → 가중평균={S_final/sum_w:.6f} (목표≈{B_target/sum_w:.6f})"
    )
