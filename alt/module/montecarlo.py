...
try:
    from cvx.step1_3c_opt import parse_sensitivity
except ImportError:
    from cvx.step1_3c_opt import parse_sensitivity
...
def load_sensitivity_csv(
    ...
    names, c_vals, w_vals = parse_sensitivity(path, c_col, w_col)
    return names, c_vals, w_vals
...
def load_alpha_csv(path: str, target_bit: int) -> Dict[str, float]:
    ...
    with open(path, "r", encoding="utf-8") as f:
        reader = csv.DictReader(f)
        for row in reader:
            raw_bit = (row.get("bit") or row.get("bits") or row.get("R_int") or "").strip()
            if not raw_bit:
                continue
            try:
            ...
            raw_alpha = (row.get("alpha") or "").strip()
            if not raw_alpha:
                continue
            try:
                amap[name] = float(raw_alpha)
            except Exception:
                continue
    return amap
...
def get_initial_seed(C_prime_map, W_map, avg_bits, bmin=1, bmax=4) -> Dict[str, int]:
    ...
    w_map = {n: int(W_map[n]) for n in names}
    if sum(w_map.values()) == 0:
        print("[경고] 초기 시드 생성 실패: 유효한 가중치 합이 0입니다.", flush=True)
        return {}

    base_bit = int(round(float(avg_bits)))
    base_bit = max(int(bmin), min(int(bmax), base_bit))
    b_seed = {n: base_bit for n in names}
    B_target = target_weighted_sum(float(avg_bits), w_map)
