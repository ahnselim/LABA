...
목적:
  ...
  에 대해 sparse+dense split이 rank-friendly한지 검증한다.
...
각 레이어마다:
  ...
  7) 최종 복원:
         R_d_hat = S + D_r
  8) 지표 계산:
       - dense_evr_at_rank = ||D_r||_F^2 / ||D||_F^2
       - weighted_loss_after_rank = ||R_d - R_d_hat||_F^2 = ||D - D_r||_F^2
       - total_explained_ratio = ||R_d_hat||_F^2 / ||R_d||_F^2
       - stable_rank_dense
       - sparse_fraction
...
예시:
CUDA_VISIBLE_DEVICES=2 python test/verify_sparse_split.py \
  --model_id meta-llama/Llama-3.1-8B \
  --step1_dir ./output/llama3_8b/step1_quant/2bit \
  --calib_s_path ./output/llama3_8b/calib_sqrtdiag.pt \
  --out_dir ./output/llama3_8b/verify_sparse_split/2bit_attn \
  --rank 64 \
  --layer_regex "self_attn" \
  --device cuda \
...
def _global_topk_mask_abs(X: torch.Tensor, ratio: float) -> torch.Tensor:
    """
    ratio: 0.001 means top 0.1% abs entries
    """
    flat = X.abs().flatten()
    n = flat.numel()
    if n == 0 or ratio <= 0:
...
def _row_topk_mask_abs(X: torch.Tensor, ratio: float) -> torch.Tensor:
    ...
    k = max(1, int(math.ceil(float(ratio) * n)))
    k = min(k, n)
    idx = torch.topk(X.abs(), k=k, dim=1, largest=True, sorted=False).indices  # [m,k]
...
def _split_sparse_dense(
    ...
    """
    returns S, D, mask such that X = S + D
    """
    if mode == "none":
        mask = torch.zeros_like(X, dtype=torch.bool)
...
@torch.no_grad()
def _evaluate_mode(
    X: torch.Tensor,       # shaped residual R_d
    mode: str,
    ratio: float,
    rank: int,
) -> Dict[str, float]:
...
# -------------------------------------------------
def main():
    ap = argparse.ArgumentParser("Verify sparse+dense split on Hessian-shaped residual")
    ...
    ap.add_argument("--topk_ratios", type=float, nargs="+", default=[0.001, 0.005, 0.01],
                    help="0.001 = top 0.1%%")
    ...
    print(f"[Verify-SparseSplit] loading step1 artifacts from: {args.step1_dir}")
    ...
    print(f"[Verify-SparseSplit] loading calib: {args.calib_s_path}")
    calib_map = _load_calib_sqrtdiag_map(args.calib_s_path)

    print(f"[Verify-SparseSplit] loading model state: {args.model_id}")
    try:
        ...
        print("[Verify-SparseSplit] meta tensor detected under device_map mode, reloading on CPU only.")
        state = _load_model_state_cpu(
            model_id=args.model_id,
            revision=args.revision,
            trust_remote_code=args.trust_remote_code,
    ...
    print(f"[Verify-SparseSplit] selected layers: {len(target_layers)}")
    ...
    for idx, full_name in enumerate(tqdm(target_layers, desc="layers")):
        if full_name not in codebooks or full_name not in qcodes_dict or full_name not in metas:
            print(f"[Verify-SparseSplit][warn] missing step1 artifact for {full_name}; skipping")
            continue

        W_cpu = state[full_name].to(torch.float32)
        d_cpu = _lookup_sqrt_diag(calib_map, full_name)
        if d_cpu is None:
            print(f"[Verify-SparseSplit][warn] missing sqrt diag for {full_name}; skipping")
            continue
        ...
        if d.numel() != W.shape[1]:
            print(
                f"[Verify-SparseSplit][warn] sqrt-diag size mismatch for {full_name}: "
    ...
    print("[Verify-SparseSplit] saved:")
    print(f"  - {csv_path}")
    print(f"  - {json_path}")
    ...
    for group_name in ("overall", "attention", "mlp"):
        ...
        if not group:
            continue
        print(f"\n[{group_name}]")
        items = sorted(group.items(), key=lambda x: x[1]["weighted_loss_after_rank"])
        for name, item in items[:12]:
            print(
                f"  {name:42s} | dense_EVR@{args.rank}={item['dense_evr_at_rank']:.6f} "
                f"| weighted_loss={item['weighted_loss_after_rank']:.6f} "
                f"| total_explained={item['total_explained_ratio']:.6f} "
                f"| sparse_frac={item['sparse_fraction']:.6f} "
                f"| sparse_energy={item['sparse_energy_ratio']:.6f}"
            )
        if items:
            print(f"  -> best by weighted_loss: {items[0][0]}")
