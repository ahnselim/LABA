...
CUDA_VISIBLE_DEVICES=0 python step_2_calib.py \
  --model_name meta-llama/Llama-3.2-3B \
  --out_calib_s ./output/llama3_3b_64/calib_sqrtdiag.pt \
  ...
  --num_gpus 2 \
  --eps 1e-6 \
  --cov_mode second_moment \
  --bits 2 \
  --group_size 128
  
CUDA_VISIBLE_DEVICES=0 python step_2_calib.py \
  --model_name meta-llama/Llama-3.2-3B \
  --out_calib_s ./output/llama3_3b/calib_sqrtdiag.pt \
  --dataset DKYoon/SlimPajama-6B \
  --split train \
  --use_streaming true \
  --seq_len 2048 \
    ...
    if str(args.device_map).strip().lower() == "auto" and int(args.num_gpus) > 0 and torch.cuda.is_available():
        ...
        if use_n > 0:
            ...
            for idx in range(use_n):
                ...
                reserve_gib = int(args.first_gpu_reserve_gib) if idx == 0 else int(args.other_gpu_reserve_gib)
            ...
            print(f"[Calib] max_memory: {max_memory}")
...
class Step02CalibConfig:
    ...
    device_map: str = "auto"
    num_gpus: int = 2
    first_gpu_reserve_gib: int = 0
    other_gpu_reserve_gib: int = 1
...
def build_command(cfg: Step02CalibConfig) -> List[str]:
    cmd: List[str] = [
        ...
        "--first_gpu_reserve_gib",
        str(int(cfg.first_gpu_reserve_gib)),
        "--other_gpu_reserve_gib",
        str(int(cfg.other_gpu_reserve_gib)),
        "--cov_mode",
        str(cfg.cov_mode),
        "--eps",
        str(float(cfg.eps)),
