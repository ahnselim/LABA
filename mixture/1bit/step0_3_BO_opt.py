...
# -*- coding: utf-8 -*-
"""
Mixture step0 module: Step0-3 BO + full optimization runner (1-bit delta + uv + ab).
...
@torch.no_grad()
def project_delta_zero_mean_if_1bit(
    delta_ogq: torch.Tensor, bits: int, q_levels: int
) -> None:
    """
    For the 1-bit codebook C=[mu-beta, mu+beta], remove the common shift term
    so delta only adjusts the level gap instead of redundantly moving the center.
    """
    if int(bits) != 1 or int(q_levels) != 2:
        return
    delta_ogq.sub_(delta_ogq.mean(dim=-1, keepdim=True))
...
def load_context(args: argparse.Namespace) -> dict:
    ...
    for key, meta in metas.items():
        ...
        if bits != 1 or levels != 2:
            raise ValueError(
                f"{key} has incompatible step1 meta for LABA/mixture/1bit: bits={bits}, levels={levels}. "
                "Rebuild step1 artifacts with the fixed 1-bit group-wise (mu,beta) quantizer."
            )
    ...
    *,
    ...
    try:
        for li, key in enumerate(keys):
            ...
            # Step1 provides the fixed 1-bit base codebook C=[mu-beta, mu+beta].
            # Step3.5 optimizes only a small offset Delta on top of that codebook.
            ...
            try:
                for outer in range(int(args.outer_loops)):
                    ...
                    for _ in range(max(1, int(args.delta_steps))):
                        ...
                        opt.step()
                        project_delta_zero_mean_if_1bit(delta, bits=bits, q_levels=q_levels)
