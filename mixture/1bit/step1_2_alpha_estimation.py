...
In the ablation directory, bit-1 may come from the group-wise `(mu,beta)` 1-bit pipeline
while bits 2/3/4 still come from the baseline prebake pipeline. This script supports that
via per-bit prebake root overrides.
"""
...
class Step12AlphaPrebakeConfig:
    ...
    prebake_root_bit1: Optional[str] = None
    prebake_root_bit2: Optional[str] = None
    prebake_root_bit3: Optional[str] = None
    prebake_root_bit4: Optional[str] = None
    calib_batch_size: int = 1
...
def _resolve_prebake_root(cfg: Step12AlphaPrebakeConfig, bit: int) -> Path:
    override = getattr(cfg, f"prebake_root_bit{int(bit)}", None)
    root = override if override is not None else cfg.prebake_root
    path = Path(str(root))
    if not path.exists():
        raise FileNotFoundError(f"prebake root not found for bit={bit}: {path}")
    return path
...
def run(cfg: Step12AlphaPrebakeConfig) -> Dict[str, str]:
    ...
    resolved_prebake_roots = {int(b): _resolve_prebake_root(cfg, int(b)) for b in bits}
    ...
    with open(csv_path, "w", newline="", encoding="utf-8") as f_main:
        ...
        for full_name, W_param in tqdm(model.named_parameters(), desc="[Step1-2] Target Weights"):
            ...
            for b in bits:
                pb_root = resolved_prebake_roots[int(b)]
                pb_file = pb_root / f"bit{b}" / f"{_safe_name(mod_name)}.pt"
    ...
    summary = {
        ...
        "revision": cfg.revision,
        "prebake_root": str(cfg.prebake_root),
        "resolved_prebake_roots": {str(k): str(v) for k, v in resolved_prebake_roots.items()},
        "bits": bits,
...
def _parse_args(argv: Optional[Sequence[str]] = None) -> Step12AlphaPrebakeConfig:
    ...
    ap.add_argument("--prebake_root", required=True, help="default prebake root used for any bit without an override")
    ...
    ap.add_argument("--prebake_root_bit1", default=None, help="override prebake root for bit1")
    ap.add_argument("--prebake_root_bit2", default=None, help="override prebake root for bit2")
    ap.add_argument("--prebake_root_bit3", default=None, help="override prebake root for bit3")
    ap.add_argument("--prebake_root_bit4", default=None, help="override prebake root for bit4")
