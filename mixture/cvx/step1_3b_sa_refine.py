...
이산 탐색으로 정수 비트 {1,2,3,4} 할당(또는 `--bits` 지정)을 **예산형(budget)** 또는 **타깃손실형(target)**에서
더 최적화합니다.

핵심 아이디어
--------------
1) 가능한 경우, 각 레이어 j에 대해 b∈bits에 대한 **실측 잔여 테이블** L_j(b)를 사용.
...
입력 CSV(유연 파싱):
  ...
  • --loss_table_csv (선택): layer_name, b1/b2/... 또는 L1/L2/... 또는 loss1/loss2/...
...
python cvx/step1_3b_sa_refine.py \
    --sens_csv ./output_7b/output_step1_cvx/step1_1/layerwise_sensitivity.csv \
    --alpha_csv ./output_7b/output_step1_cvx/step1_2/alpha_layerwise_rankvar.csv \
  --mode budget --avg_bits 2.00 \
  --steps_per_temp 2000  --max_temps 400   --rho 0.995   --restarts 2 \
  --output_dir ./output_7b/output_step1_cvx/step3b_refine
...
def load_loss_table(path: str, bits: Tuple[int, ...]) -> Dict[str, Dict[int, float]]:
    """Expected columns: layer_name + b1/b2/... or L1/L2/... or loss1/loss2/..."""
    rows = _read_csv_rows(path)
    if not rows:
    ...
    for col in rows[0].keys():
        ...
        if not m:
            continue
        bit = int(m.group(1))
        if bit not in bit_col:
            bit_col[bit] = col

    missing = [b for b in bits if b not in bit_col]
    if missing:
        raise ValueError(
            f"loss_table_csv missing columns for bits={missing}; "
            f"available_bits={sorted(bit_col.keys())}"
        )
...
def build_loss_table(
    ...
    if loss_table_csv:
        by_name = load_loss_table(loss_table_csv, bits=bits)
        for L in layers:
...
def build_cprime_for_seed(
    layers: List[Layer],
    alpha_csv: Optional[str],
    alpha_default: float,
    alpha_bit: int,
) -> List[float]:
    """Build C' used for continuous seeding (compatible with Step3 alpha_bit logic)."""
    if alpha_csv and os.path.exists(alpha_csv):
        alpha_map = load_alpha_table(alpha_csv)
        g = alpha_map.get("__GLOBAL__", {})
        global_alpha = float(g.get(alpha_bit, alpha_default))
        out: List[float] = []
        for L in layers:
            a = float(alpha_map.get(L.name, {}).get(alpha_bit, global_alpha))
            out.append(float(L.C) * a)
        return out
    return [float(L.C) * float(alpha_default) for L in layers]


def parse_bits_arg(bits_arg: str) -> Tuple[int, ...]:
    bits = tuple(sorted(set(int(x.strip()) for x in bits_arg.split(",") if x.strip())))
    if not bits:
        raise ValueError("--bits must contain at least one integer bit.")
    return bits


def nearest_allowed_bit(value: float, bits: Tuple[int, ...]) -> int:
    return min(bits, key=lambda b: (abs(float(value) - float(b)), b))


def prev_allowed_bit(bit: int, bits: Tuple[int, ...]) -> Optional[int]:
    try:
        i = bits.index(bit)
    except ValueError:
        return None
    return bits[i - 1] if i > 0 else None


def next_allowed_bit(bit: int, bits: Tuple[int, ...]) -> Optional[int]:
    try:
        i = bits.index(bit)
    except ValueError:
        return None
    return bits[i + 1] if i + 1 < len(bits) else None


def load_init_assignment(
    path: str, layers: List[Layer], bits: Tuple[int, ...]
) -> Optional[List[int]]:
    ...
    bit_col_candidates = ["b_init", "R_int", "b_final", "bit", "bits", "selected_bit"]
    bit_col = next((c for c in bit_col_candidates if c in rows[0]), None)
    if bit_col is None:
        return None

    seed = [bits[0]] * len(layers)
    filled = [False] * len(layers)
    ...
    for r in rows:
        ...
        if nm not in name_to_idx:
            continue
        try:
            raw_bit = float(r[bit_col])
        except Exception:
            continue
        j = name_to_idx[nm]
        seed[j] = nearest_allowed_bit(raw_bit, bits)
        filled[j] = True
...
def round_and_fix_budget(
    ...
    n = len(R)
    b = [nearest_allowed_bit(Rj, bits) for Rj in R]
    ...
    if B_now > target:
        while B_now > target + 1e-9:
            candidates = []
            for j in range(n):
                prev_b = prev_allowed_bit(b[j], bits)
                if prev_b is not None:
                    dL = loss_table[j][prev_b] - loss_table[j][b[j]]
                    eff = dL / max(1.0, w[j])
                    candidates.append((eff, dL, j, prev_b))
            if not candidates:
                break
            candidates.sort(key=lambda x: (x[0], x[1]))
            _, _, jbest, bnew = candidates[0]
            B_now += w[jbest] * (bnew - b[jbest])
            b[jbest] = bnew
        return b
    else:
        while B_now < target - 1e-9:
            candidates = []
            for j in range(n):
                next_b = next_allowed_bit(b[j], bits)
                if next_b is not None:
                    dL = loss_table[j][next_b] - loss_table[j][b[j]]
                    eff = dL / max(1.0, w[j])
                    candidates.append((eff, dL, j, next_b))
            if not candidates:
                break
            candidates.sort(key=lambda x: (x[0], x[1]))
            _, _, jbest, bnew = candidates[0]
...
def propose_swap_budget(
    ...
    for _ in range(64):
        ...
        if j == k:
            continue
        b_j_next = next_allowed_bit(b[j], bits)
        b_k_prev = prev_allowed_bit(b[k], bits)
        if b_j_next is not None and b_k_prev is not None:
            d = (problem.loss_table[j][b_j_next] - problem.loss_table[j][b[j]]) + (
                problem.loss_table[k][b_k_prev] - problem.loss_table[k][b[k]]
            )
            return (j, k), d
    return None, 0.0
        ...
        for _ in range(steps_per_temp):
            ...
            if Ediff < 0 or rng.random() < math.exp(-Ediff / max(1e-12, T)):
                ...
                if E < best_E - 1e-12 or (
                    ...
                    best_E, best_L, best_B = E, L, B
                    best_b = b[:]
        lam = lam * (lam_up if L > L_tgt else lam_down)
        T *= rho
        no_improve = no_improve + 1 if accept_cnt == 0 else 0
        if no_improve >= 8:
            break
...
def main():
    ...
    ap.add_argument(
        ...
        type=str,
        default=None,
        help="Measured per-layer loss table CSV with columns b1/b2/... or L1/L2/... or loss1/loss2/...",
    )
    ap.add_argument(
        "--alpha_csv",
        type=str,
    ...
    ap.add_argument(
        ...
        type=int,
        default=3,
        help="Reference alpha bit used to build C' for continuous seeding (default: 3).",
    )
    ...
    ap.add_argument(
        ...
        type=str,
        default="1,2,3,4",
        help="Allowed bits, comma-separated (default: 1,2,3,4)",
    )
    ...
    loss_table, _alpha_global = build_loss_table(
        layers=layers,
        bits=bits,
    ...
    # Build Cprime for continuous seeding (compatible with Step3 alpha_bit behavior)
    Cprime = build_cprime_for_seed(
        layers=layers,
        alpha_csv=args.alpha_csv,
        alpha_default=args.alpha_default,
        alpha_bit=int(args.alpha_bit),
    )
    init_seed = load_init_assignment(args.init_assign_csv, layers, bits)
    ...
    if args.mode == "budget":
        if args.avg_bits is not None:
            ...
            B = float(total_bits(problem, init_seed))
        else:
            raise ValueError(
                "budget mode requires --avg_bits or --total_bits "
                "(or provide --init_assign_csv to infer budget)."
            )
        ...
        if init_seed is not None:
            b_seed = init_seed[:]
            # fix to requested/inferred exact budget
        ...
        for _ in range(max(1, args.restarts)):
            ...
            if LL < best_L:
                best_L, best_b = LL, bb
        final_b = best_b
        # Keep budget compatibility with Step3 output settings.
        if abs(total_bits(problem, final_b) - B) > 1e-6:
            final_b = round_and_fix_budget(
                [float(x) for x in final_b], w, loss_table, bits, B
            )
        final_L = total_loss(problem, final_b)
        final_B = total_bits(problem, final_b)
        if args.avg_bits is not None:
            target_desc = f"avg_bits={args.avg_bits}"
        elif args.total_bits is not None:
            target_desc = f"total_bits={args.total_bits}"
        else:
            target_desc = f"total_bits(inferred_from_seed)={B:.6f}"

    else:
        if args.target_loss is not None:
            L_tgt = float(args.target_loss)
        elif init_seed is not None:
            L_tgt = float(total_loss(problem, init_seed))
        else:
            raise ValueError(
                "target mode requires --target_loss "
                "(or provide --init_assign_csv to infer target from seed loss)."
            )
    ...
    with open(report_path, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        loss_cols = [f"L{b}" for b in bits]
        writer.writerow(
            ["layer_name", "w_j", *loss_cols, "b_init_or_seed", "b_final", "loss_final", "Δloss_final"]
        )
        report_seed = locals().get("b_seed", final_b)
        for j, Lyr in enumerate(problem.layers):
            row_losses = [problem.loss_table[j][b] for b in bits]
            b0 = report_seed[j] if j < len(report_seed) else final_b[j]
            bf = final_b[j]
            lf = problem.loss_table[j][bf]
            dl = lf - problem.loss_table[j][b0]
            writer.writerow([Lyr.name, Lyr.w, *[f"{v:.6e}" for v in row_losses], b0, bf, f"{lf:.6e}", f"{dl:.6e}"])
        Wsum = sum(w)
    ...
    if args.out_csv:
        out_dirname = os.path.dirname(args.out_csv)
        if out_dirname:
