...
python montecarlo/real_ppl/step3b_ppl_mc_converge_round.py \
  ...
  --output_dir ../artifacts/montecarlo/step3b_ppl_roundband > llama3_3b_ppl_roundband.log 2>&1 &
...
def _detect_src_dir(curr_dir: Path) -> Path:
    """
    Locate the src/ directory that hosts step2_alpha_estimation.py even if this
    script is copied elsewhere (e.g., proxy_codes/). We search upward for a
    direct hit as well as nested src/ directories.
    """
...
# Ensure repo src directory (and current dir) stay importable even after moving the file.
...
try:
    ...
    _canonical_dataset_name = _step2_alpha._canonical_dataset_name
except ImportError as e:
