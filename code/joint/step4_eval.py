...
# -*- coding: utf-8 -*-
"""
STEP 4 Eval (for Step2.5/Step3 outputs) - Wdq* baseline + AB* correction (Layerwise, no grouping, no act-quant)

목적:
  - Step2.5/Step3가 저장한 Wdq_star(=최적화된 dequant weight)와 AB 계열 보정값을 HF 모델에 주입
  - (1) Wdq*-only baseline
  - (2) Wdq* + AB* correction
    PPL 및 (선택) generation latency/throughput 비교

입력 (Step2.5/Step3 산출물):
  --original_weights_path : (optional) original fp16 weights partial/full .pt (Step1의 original_weights*.pt)
  --wdq_star_path         : wdq_star.pt  { "<module>.weight": Tensor[O,I] } (fp16/cpu)
  --low_rank_ab_path      : (optional) low_rank_ab.pt { "<module>.weight": {"A":..., "B":...} }
                           또는 Step3 lowrank_uv_ab.pt {A,B,u,v}; 이 경우 calib_s_path 필요(inv_s)
  --low_rank_abbar_path   : (optional) low_rank_abbar.pt { "<module>.weight": {"A":..., "Bbar":...} }
  --calib_s_path          : (required for low_rank_abbar-diag or Step3 uv-ab) Step2 calib_sqrtdiag.pt (inv_s 필요)
  --calib_h_lowrank_path  : (required only if low_rank_abbar_path uses lowrank H) calib_h_lowrank.pt (U, sqrt_lambda 필요)

사용 예:
CUDA_VISIBLE_DEVICES=2 python step4_eval.py \
  --model_name meta-llama/Llama-3.2-3B \
  --wdq_star_path ./output/4bit/joint_soft_rank64/wdq_star.pt \
  --low_rank_ab_path ./output/4bit/joint_soft_rank64/low_rank_ab.pt \
  --device cuda:0 \
  --skip_gen
  
CUDA_VISIBLE_DEVICES=1 python step4_eval.py \
  --model_name meta-llama/Llama-3.2-3B \
  --wdq_star_path ./output/3bit_step3_delta_uv_ab_agg_v3/wdq_star_best.pt \
  --low_rank_ab_path ./output/3bit_step3_delta_uv_ab_agg_v3/lowrank_uv_ab_best.pt \
  --calib_s_path ./output/3bit_asym_nonuniform_base/calib_sqrtdiag.pt \
  --device cuda:0 \
  --skip_gen

CUDA_VISIBLE_DEVICES=2 python step4_eval.py \
  --model_name meta-llama/Llama-3.2-3B \
  --wdq_star_path ./output/4bit_output_bo/4bit_bo_mse_only_20260219_024626/trial_0019/wdq_star_best.pt \
  --low_rank_ab_path ./output/4bit_output_bo/4bit_bo_mse_only_20260219_024626/trial_0019/lowrank_uv_ab_best.pt \
  --calib_s_path ./output/2bit_asym_nonuniform_base/calib_sqrtdiag.pt \
  --device cuda:0 \
  --skip_gen
...
def dequant_1bit_mu_beta(W: torch.Tensor, group_size: int):
    ...
    Wg_orig, O, G, gs, orig_I = _to_groups(W32, group_size)
    beta = Wg_orig.abs().mean(dim=-1, keepdim=True)
    mu = W32.mean()
    W_center = W32 - mu
...
def main():
    ...
    if args.wdq_star_path is not None:
        ...
        if args.bits <= 2:
            print(f"⚠️ bits={args.bits} uses baseline 1/2-bit quant; theta params will be ignored.")
