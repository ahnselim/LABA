...
Two-stage optimization runner for delta+uv+ab:
  Stage1: BO on first N transformer blocks (fast hyperparameter search)
  Stage2: full-layer optimization with Stage1 best hyperparameters (fixed)

Saved artifacts are directly usable in step4_eval.py:
  - wdq_star_best.pt
  - lowrank_uv_ab_best.pt
  
CUDA_VISIBLE_DEVICES=2 nohup python step3_5_v2.py \
  --model_id meta-llama/Llama-3.2-3B \
  --step1_dir ./output/1bit_asym_nonuniform_base \
  --calib_s ./output/3bit_asym_nonuniform_base/calib_sqrtdiag.pt \
  --out_root ./output/1bit_output_bo \
  --device cuda \
  --stage1_max_blocks 8 \
  --stage1_n_trials 20 \
  --rank_ab 64 \
  --outer_loops 40 \
  --delta_steps 8 \
  --study_name 1bit_bo_block8_v2 \
  --trust_remote_code > ./logs/1bit_bo_topk.log 2>&1 &
